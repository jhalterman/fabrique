...
public class InterceptorTest {
    ...
    /**
     * Tests that the cached injector does not prevent an interceptor from being applied later for
     * the same type.
     */
    @Test
    public void testCachedInjectorCollision() {
        ...
        ObjectFactory.loadModules(new AbstractModule() {
            protected void configure() {
                bindInterceptor(Matchers.only(Interceptable.class),
                        Matchers.returns(only(List.class)), returnNullInterceptor);
            }
        });
    ...
    public void testClassInterceptor() {
        ObjectFactory.loadModules(new AbstractModule() {
            protected void configure() {
                bindInterceptor(Matchers.annotatedWith(Adder.class), Matchers.any(),
                        additionInterceptor);
            }
        });
    ...
    public void testMethodInterceptor() {
        ObjectFactory.loadModules(new AbstractModule() {
            protected void configure() {
                bindInterceptor(Matchers.any(), Matchers.annotatedWith(Adder.class),
                        additionInterceptor);
            }
        });
    ...
    public void testMultipleInterceptorsForMethod() {
        ObjectFactory.loadModules(new AbstractModule() {
            protected void configure() {
                bindInterceptor(Matchers.any(), Matchers.returns(only(List.class)),
                        countingInterceptor);
                bindInterceptor(Matchers.any(),
                        Matchers.returns(only(List.class).or(only(Map.class))),
